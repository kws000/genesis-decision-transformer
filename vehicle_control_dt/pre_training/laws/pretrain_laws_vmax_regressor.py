...
class CSVDataset(Dataset):
    def __init__(self, csv_path: str):
        ...
        self.x_cols = ["radius_m", "mu", "curvature_1pm"]  # ★ 3特徴
        X = df[self.x_cols].to_numpy(np.float32)
        # 因子分解ターゲット: v_base = vmax_mps / (sqrt(g) * safety)
        denom = np.sqrt(df["g_mps2"].to_numpy(np.float32)) * df["safety_margin"].to_numpy(np.float32)
        y = (df["vmax_mps"].to_numpy(np.float32) / np.maximum(denom, 1e-6)).reshape(-1, 1)
        self.X, self.y = X, y
        # 標準化（堅牢：極小stdをクリップ）
        self.x_mean = self.X.mean(0, keepdims=True).astype(np.float32)
        self.x_std  = np.maximum(self.X.std(0, keepdims=True).astype(np.float32), 1e-3)
